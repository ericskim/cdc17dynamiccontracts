...
for k,v in upstream.items(): # check
    print k,":\t",v
...
# AG_cost=pickle.load( open( "contracts_costs.txt", "rb" ) )
(inv,u)=pickle.load( open( "invaraint_automata.txt", "rb" ) )


#(X_initial,Controls,Demands,Outputs,cost, state)
#    0          1       2        3     4     5
# for k,v in AG[2,3][1].items():
#   print k,v
# print "cost is",AG[2,3][4]
...
"""
    ...
    (X_initial,Controls,Demands,Outputs,cost, state )
        0        1         2      3      4      5
    AG[network_index, contract indexe]=X_initial,Controls,Demands,Outputs,cost
"""
    


def evolve(contract_vector,t):
    for link_address, link_object in L.items():
        network_index=link_address[0]
#       for key,val in AG[network_index,contract_vector[network_index-1]][1].items(): # Controls
        if link_object.type!="sink":
            link_object.u=AG[network_index,contract_vector[network_index-1]][1][link_address,t]
...
def evolve_T():
    ...
    for t in range(0,T):
        ...
        for i in [1,2,3]:
            ...
            g.write("%d %d "%(L[(i,1,"West")].u,L[(i,2,"North")].u))
            g.write("%d %d "%(L[(i,2,"West")].u,L[(i,1,"South")].u))
            g.write("%d %d "%(L[(i,1,"South")].u,L[(i,2,"West")].u))
...
print "Total delay NO MPC is", Total_Delay 
raise "EnD"
"""
  
            
        
# contract_vector=inv[5]
# for t in range(0,6):
#   evolve(contract_vector,t)
# 
# for l in L.values():
...
# MPC_Contract(self,T,clock,U_history,X_contract,D_contract,Output_Contract)        

"""
    AG Tuple form:
    (X_initial,Controls,Demands,Outputs,cost, state )
        0        1         2      3      4      5
    AG[network_index, indexes]=X_initial,Controls,Demands,Outputs,cost
"""
# for k,v in AG[1,3][1].items():
#     print k,v
# raise "STOPPED"
U_history={}
